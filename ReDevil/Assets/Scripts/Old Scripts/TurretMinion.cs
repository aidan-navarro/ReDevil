...
//bad naming for the class.  this is for the nodeppou enemy.  fires a projectile that moves in a horizontal line toward the enemy
public class TurretMinion : BaseMinion
{
    ...
    private void Start()
    {
        fireLeft = new Vector2(-1, 0);
        fireRight = new Vector2(1, 0);
    }
        ...
        if(attack == true)
        {
            ...
            if (attackTimer >= shootInterval)
            {
                ...
                if (!attackRight)
                {
                    ...
                    bulletClone.GetComponent<Rigidbody2D>().velocity = fireLeft * bulletSpeed;
                ...
                if (attackRight)
                {
                    ...
                    GameObject bulletClone;
                    bulletClone = Instantiate(bullet, shootPoint.transform.position, shootPoint.transform.rotation) as GameObject;
                    bulletClone.GetComponent<Rigidbody2D>().velocity = fireRight * bulletSpeed;
